...
public class KafkaEventsConsumer {
  ...
  public void handleMovieCreated(MovieCreatedEvent event) {
    MovieReadModel readModel =
        MovieReadModel.builder()
            ...
            .movieName(
                new MovieReadModel.MovieName(event.enTitle(), event.chTitle(), event.keyword()))
            .genres(event.genres())
  ...
  public void handleShowtimeCreated(ShowtimeCreatedEvent event) {
    Optional<MovieReadModel> movieRead = movieReadRepository.findById(event.movieId());

    Optional<ShowTime> showTime = showtimeRepository.findByShowTimeId(event.showtimeId());
    ...
    if (movieRead.isPresent() && showTime.isPresent()) {
      ...
      Theater theater =
          theaterRepository
              .findByTheaterId(show.getTheater().getTheaterId())
              .orElseThrow(() -> new IllegalStateException("Theater not found"));

      Screen screen =
          screenRepository
              .findByScreenId(show.getScreen().getScreenId())
      ...
      logger.warn(
          "Movie ID [{}] not found for showtime. Might be eventual consistency issue.",
          event.movieId());
    }
  }
